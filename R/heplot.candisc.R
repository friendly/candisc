...
#' Passing an expanded range in `zlim` to \code{\link[heplots]{heplot3d}}
#' usually helps.
#' 
#' @aliases heplot.candisc heplot3d.candisc
#' @param mod A `candisc` object for one term in a `mlm`
...
#'        defaults, `asp=1` for `heplot.candisc` and `asp="iso"` for
#'        `heplot3d.candisc` ensure equal units on all axes, so that angles and
...
#'        plot as a circle (sphere) in canonical space.  For `heplot3d.candisc`,
...
#' @param terms Terms from the original `mlm` whose H ellipses are to be
#' plotted in canonical space.  The default is the one term for which the
#' canonical scores were computed.  If `terms=TRUE`, all terms are
#' plotted.
#' @param \dots Arguments to be passed down to \code{\link[heplots]{heplot}} or
#' \code{\link[heplots]{heplot3d}}
#' @return `heplot.candisc` returns invisibly an object of class
...
#' @param mod A `candiscList` object for terms in a `mlm`
#' @param term The name of one term to be plotted for the `heplot` and
#' `heplot3d` methods. If not specified, one plot is produced for each
#' term in the `mlm` object.
#' @param ask If `TRUE` (the default), a menu of terms is presented; if
