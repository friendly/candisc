...
#' @param xlim    numeric vector of length 2 giving the x-axis limits. If `NULL` (default), uses the 
#'                range of the x variable in the data.
#' @param ylim    numeric vector of length 2 giving the y-axis limits. If `NULL` (default), uses the 
#'                range of the y variable in the data.
...
#' 
#' Generalized by Michael Friendly & Claude
#' @references 
...
#' 
#' # Reverse both axes in discriminant space
#' plot_discrim(iris.lda, LD2 ~ LD1, rev.axes = c(TRUE, TRUE))
#' 
#' # Control axis limits
#' plot_discrim(iris.lda, Petal.Length ~ Petal.Width,
#'              xlim = c(0, 3), ylim = c(0, 8))
#' 
#' # Zoom in on a region in discriminant space
#' plot_discrim(iris.lda, LD2 ~ LD1,
...
plot_discrim <- function(
  ...
  if (is_discrim_space) {
    ...
    scores_data <- predict_discrim(model, data)
    ...
    svd_vals <- model$svd
    variance_props <- (svd_vals^2) / sum(svd_vals^2) * 100
