...
The `Diabetes` data set is contained in the `heplots` package.  This vignette uses this data set to illustrate various graphical methods for multivariate linear models. As we'll see, the data is peculiar in several respects and a standard MANOVA
...
This can be assessed visually with a Chi-square QQ plot, that plots 
...
```{r diab-can-plot, fig.width=6, fig.height=4}
...
```{r diab-heplot, , fig.width=6, fig.height=4}
heplot(diab.can, fill=c(TRUE, FALSE), fill.alpha=0.1, var.lwd=2)
```

## Other analyses

There are other methods that could be used to analyse these data.
